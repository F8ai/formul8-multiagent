...
const agents = {
    f8_agent: {
        ...
        getResponse: (message) => {
            ...
            if (lower.includes('diagram') || lower.includes('flowchart') || lower.includes('mermaid')) {
                return `Here's a flowchart showing our cannabis product development process:
...
This diagram illustrates the key stages from initial concept to market release. Would you like more details on any specific stage?`;
            }
...
app.get('/', (_req, res) => {
    ...
    <body>
        ...
        <script>
            ...
            async function addMessage(content, type, agent = null, usage = null) {
                ...
                while ((match = mermaidRegex.exec(content)) !== null) {
                    mermaidMatches.push(match[1]);
                }
                
                if (mermaidMatches.length > 0) {
                    // Replace mermaid code blocks with placeholders
                    processedContent = content.replace(mermaidRegex, (_, code, index) => {
                        const id = 'mermaid-' + Date.now() + '-' + mermaidMatches.indexOf(code);
                        return \`<div class="mermaid-diagram"><div class="mermaid-container" id="\${id}"></div></div>\`;
                    });
                }
                
                let messageContent = \`<strong>\${agent || 'Formul8 AI'}:</strong> \${processedContent}\`;
                if (usage) {
                ...
                if (mermaidMatches.length > 0 && typeof mermaid !== 'undefined') {
                    for (let i = 0; i < mermaidMatches.length; i++) {
                        const id = 'mermaid-' + (Date.now() - (mermaidMatches.length - i)) + '-' + i;
                        const container = document.getElementById(id);
                        if (container) {
                            try {
                                const { svg } = await mermaid.render('diagram-' + id, mermaidMatches[i]);
...
app.get('/health', async (_req, res) => {
    ...
    const microserviceResults = await Promise.allSettled(microservices.map(service => checkMicroservice(service)));
 ...
 */
app.post('/api/query/:agent', async (req, res) => {
