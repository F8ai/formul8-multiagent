...
API_KEY=$(gh secret get OPENROUTER_API_KEY 2>/dev/null || echo "")

if [ -z "$API_KEY" ]; then
    echo -e "${RED}Error: OPENROUTER_API_KEY not found in GitHub Secrets${NC}"
    echo "Please add it to your repository secrets:"
    echo "1. Go to your repository on GitHub"
    echo "2. Click Settings > Secrets and variables > Actions"
    echo "3. Click 'New repository secret'"
    echo "4. Name: OPENROUTER_API_KEY"
    echo "5. Value: your-openrouter-api-key"
    exit 1
fi

echo -e "${GREEN}✅ OpenRouter API key found in GitHub Secrets${NC}"
